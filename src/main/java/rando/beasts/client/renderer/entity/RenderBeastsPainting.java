...
{
    private static final ResourceLocation tex = new ResourceLocation(BeastsReference.ID,"textures/painting/beasts_paintings.png");
    ...
    public void doRender(EntityBeastsPainting entity, double x, double y, double z, float entityYaw, float partialTicks)
    {
        ...
        EntityBeastsPainting.EnumArt entityBeastsPainting$enumart = entity.art;
        float f = 0.0625F;
        ...
        this.RenderBeastsPainting(entity, entityBeastsPainting$enumart.sizeX, entityBeastsPainting$enumart.sizeY, entityBeastsPainting$enumart.offsetX, entityBeastsPainting$enumart.offsetY);
    ...
    private void RenderBeastsPainting(EntityBeastsPainting BeastsPainting, int width, int height, int textureU, int textureV)
    {
        ...
        for (int i = 0; i < width / 16; ++i)
        {
            for (int j = 0; j < height / 16; ++j)
            {
                ...
                this.setLightmap(BeastsPainting, (f15 + f16) / 2.0F, (f17 + f18) / 2.0F);
    ...
    private void setLightmap(EntityBeastsPainting BeastsPainting, float p_77008_2_, float p_77008_3_)
    {
        ...
        int j = MathHelper.floor(BeastsPainting.posY + (double)(p_77008_3_ / 16.0F));
        int k = MathHelper.floor(BeastsPainting.posZ);
        EnumFacing enumfacing = BeastsPainting.facingDirection;

        if (enumfacing == EnumFacing.NORTH)
        {
            i = MathHelper.floor(BeastsPainting.posX + (double)(p_77008_2_ / 16.0F));
        }

        if (enumfacing == EnumFacing.WEST)
        {
            k = MathHelper.floor(BeastsPainting.posZ - (double)(p_77008_2_ / 16.0F));
        }
