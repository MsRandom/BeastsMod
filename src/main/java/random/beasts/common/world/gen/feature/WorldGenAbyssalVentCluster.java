...
public class WorldGenAbyssalVentCluster implements IBeastsGenerator {
    ...
    public boolean generate(IWorld worldIn, Random rand, BlockPos position) {
        ...
        for (int i = 0; i < number; ++i) {
            ...
            if ((!worldIn.getDimension().isNether() || blockpos.getY() < 255) && maxHeight > 1 && new ChunkPos(position).equals(new ChunkPos(blockpos))) {
                ...
                BlockPos.getAllInBox(position.add(spacing, spacing, spacing), position.add(-spacing, -spacing, -spacing)).forEach(pos -> {
