...
public class RecipeCoconutJuice extends ShapelessRecipes {
    ...
    public RecipeCoconutJuice() {
        super("", new ItemStack(BeastsItems.COCONUT_JUICE), NonNullList.from(Ingredient.EMPTY, Ingredient.fromStacks(ImmutableList.copyOf(Item.REGISTRY).stream().filter(item -> item instanceof SwordItem).map(item -> new ItemStack(item, 1, OreDictionary.WILDCARD_VALUE)).toArray(ItemStack[]::new)), Ingredient.fromItem(BeastsItems.COCONUT)));
